...
class NewsClustering:
    ...
    def cluster_news_by_weighted_keywords(cls, news_articles, eps=0.35, max_size=200, by='kwords'):
        ...
        cut = len(news_list) - df.shape[0]
        if cut != 0:
            df['news'] = news_list[:-cut]
        else:
            df['news'] = news_list
    ...
    def db_scan_recursive(cls, dist_matrix=None, eps=0.25, max_size=200):
        ...
        cut = len(model.labels_) - len(core_indices)
        if cut != 0:
            results = {i: {"cluster": label, "core_index": core_indices[i]}
                    for i, label in enumerate(model.labels_[:-cut])}
        else:
            results = {i: {"cluster": label, "core_index": core_indices[i]}
        ...
        try:
            ...
            cut = len(model.labels_) - len(core_indices) + 1
            results = {i: {"cluster": label, "core_index": core_indices[i]}
                       for i, label in enumerate(model.labels_[:-cut])}
        """
