...
class NewsProcessor:
    ...
    def clean_and_extract_keywords(news):
        ...
        cleaned_title = NewsCleaner.clean_news_article(title=raw_title)
        df, df_chunk, kwords_dict = KeywordsExtract.extract_keywords(cleaned_title)
    ...
    @staticmethod
    def load_news_csv(date_start, date_finish, file): #
        _df = pd.read_csv(file) #.iloc[:] , parse_dates=[0], infer_datetime_format=True
        if file == 'data/ireland-news-headlines.csv':
            _df.columns = ["pubDate", "category", "title"]
            _df.drop(["category"], axis=1, inplace=True)
        else:
            _df.columns = ["pubDate", "title"]
            
        date_start = int(date_start)
        date_finish = int(date_finish)
        _df = _df.loc[(_df['pubDate'] <= date_finish) & (_df['pubDate'] >= date_start)]
        
        #_df = _df[:number_of_lines]
        _df['_id'] = _df.index
